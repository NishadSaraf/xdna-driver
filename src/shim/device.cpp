...
struct resource_info
{
  ...
  get(const xrt_core::device* device, key_type)
  {
    ...
    info_items[5].data_uint64 = resource_info.npu_curr_clk_max;

    // Older drivers don't populate npu_curr_clk_max in the resource info
    // payload. Fall back to the live H-clock from the clock metadata query.
    if (!resource_info.npu_curr_clk_max) {
      amdxdna_drm_query_clock_metadata clock_metadata = {};
