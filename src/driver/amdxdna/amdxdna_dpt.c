...
static irqreturn_t dpt_irq_handler(int irq, void *data)
{
	...
	spin_lock(&dpt->lock);
	amdxdna_update_tail(dpt);
	spin_unlock(&dpt->lock);
...
{
	/*
	 * Cache the current state FW polling to poll_fw_log to retain the polling state across
	 * suspend/resume
	 */
	poll_fw_log = xdna->fw_log->polling;
...
void amdxdna_dpt_enable_polling(struct amdxdna_dpt *dpt, bool enable)
{
...
int amdxdna_fw_log_init(struct amdxdna_dev *xdna)
{
	...
	strncpy(log_hdl->name, AMDXDNA_DPT_FW_LOG_NAME, sizeof(log_hdl->name));
