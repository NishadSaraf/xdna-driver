						     ...
						     enum dma_data_direction dir)
{
	...
	string_get_size(dma_hdl->aligned_size, 1, STRING_UNITS_2, print_size, sizeof(print_size));
	XDNA_DBG(xdna, "Allocated %s mgmt buffer at 0x%llx with DMA addr: 0x%llx", print_size,
		 (u64)dma_hdl->vaddr, dma_hdl->dma_hdl);
	dma_hdl->size = size;
	dma_hdl->xdna = xdna;
	dma_hdl->dir = dir;
