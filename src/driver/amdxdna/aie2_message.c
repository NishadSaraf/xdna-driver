...
static const struct kernel_param_ops force_preempt_ops = {
		.set = param_set_bool,
		.get = param_get_bool,
};

static bool force_preempt;
module_param_cb(force_preempt, &force_preempt_ops, &force_preempt, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(force_preempt, "Force preempt apps (Default false)");
...
int aie2_create_context(struct amdxdna_dev_hdl *ndev, struct amdxdna_hwctx *hwctx)
{
	...
	if (force_preempt || ndev->force_preempt_enabled) {
