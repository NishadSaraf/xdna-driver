				       ...
				       struct amdxdna_drm_get_array *args)
{
	...
	/*
	 * Use the visibility-gated emitter so the listing only includes
	 * contexts the caller is allowed to see; contexts owned by another
	 * Linux user are skipped (CAP_SYS_ADMIN sees all).
	 */
	amdxdna_for_each_client(xdna, tmp_client) {
		ret = amdxdna_hwctx_walk(tmp_client, &array_args, NULL,
					 aie2_hwctx_status_cb);
